    ...
    const getManualData = async () => {
        ...
        try {
            ...
            if (audioData.data?.data && !audioData.error) {
                ...
                let jsonTranscript = null

                try {
                    jsonTranscript = JSON.parse(question.vipListening.text)
                } catch (err) {
                    console.error("JSON نامعتبره:", err)
                    setError('JSON نامعتبره')
                }

                for (const segment of jsonTranscript.segments) {
                ...
                if (question.vipListening) {
                    setQuestion(prev => ({
                        ...
                        vipListening: {
                            ...(prev.vipListening ?? { voiceName: '', text: '' }),
                            transcription: matchedTranscript
                        }
                    }))
                }
